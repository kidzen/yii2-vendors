...
//            $pk = empty($this->tableSchema->primaryKey) ? $this->tableSchema->getColumnNames()[0] : $this->tableSchema->primaryKey[0];
    ...
    public function relationAttributes($modelName)
    {
        $model = new $modelName();
        $fks = $model->getTableSchema()->foreignKeys;
        $relAttributes = [];
        foreach ($fks as $key => $fk) {
            $relColumn = strtr(array_keys($fk)[1],['_id'=>'']);
            $humanize = \yii\helpers\Inflector::camelize($fk[0]);
            $relClass = '\\common\\models\\'.$humanize;
            $relModel = new $relClass();
            foreach ($relModel->attributes as $col => $data) {
